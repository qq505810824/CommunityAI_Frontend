...
export default function CampaignItem(props: ViewProps) {
    ...
    const handleClick = () => {
        if (product) {
            window.open(`/Campaign/${product.id}`, '_blank');
        }
    };

    const handleEdit = () => {
        if (product) router.push(`/admin/campaigns/${product.id}/edit`);
    };
