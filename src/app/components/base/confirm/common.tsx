...
const ConfirmCommon: FC<ConfirmCommonProps> = ({
    ...
    const CONFIRM_MAP: Record<string, { icon: ReactElement; confirmText: string }> = {
        danger: {
            icon: <ExclamationCircleIcon className="w-6 h-6 text-[#D92D20]" />,
            confirmText: t('common.operation.remove')
        },
        success: {
            icon: <CheckCircleIcon className="w-6 h-6 text-[#039855]" />,
    ...
    return (
        ...
        >
            <div className={cn(s[`wrapper-${type}`], 'relative p-8')}>
                <div
                    className="flex items-center justify-center absolute top-4 right-4 w-8 h-8 cursor-pointer"
                    onClick={onCancel}
                >
