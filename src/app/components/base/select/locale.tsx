...
export default function Select({ items, value, onChange }: ISelectProps) {
    ...
    return (
        <div className="w-56 text-right">
            <Menu as="div" className="relative inline-block text-left">
                <div>
                    <Menu.Button
                        className="inline-flex w-full h-[44px]justify-center items-center
...
export function InputSelect({ items, value, onChange }: ISelectProps) {
    const item = items.filter((item) => item.value === value)[0];
    return (
