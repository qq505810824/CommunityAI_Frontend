            ...
            <div className={classNames('relative')}>
                <div className="group text-gray-800">
                    {allowSearch ? (
                        ...
                        >
                            <div className="w-0 grow text-left truncate" title={selectedItem?.name}>
                                {selectedItem?.name}
                            </div>
                        </Combobox.Button>
                    )}
                ...
                {filteredItems.length > 0 && (
                    <Combobox.Options
                        className={`absolute ${isBottom ? 'bottom-full' : ''} z-10 mt-1 px-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg border-gray-200 border-[0.5px] focus:outline-none sm:text-sm ${overlayClassName}`}
                    >
    ...
    return (
        ...
        >
            <div className={`relative h-9 ${wrapperClassName}`}>
                <Listbox.Button
                    className={`w-full h-full rounded-lg border-0 bg-gray-100 py-1.5 pl-3 pr-10 sm:text-sm sm:leading-6 focus-visible:outline-none focus-visible:bg-gray-200 group-hover:bg-gray-200 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} ${className}`}
                >
                    <span
                        className={classNames(
        ...
        <PortalToFollowElem open={open} onOpenChange={setOpen} placement="bottom-start" offset={4}>
          ...
          `}
            ...
            <PortalToFollowElemContent className={`z-20 ${popupClassName}`}>
                <div className="px-1 py-1 max-h-60 overflow-auto rounded-md bg-white text-base shadow-lg border-gray-200 border-[0.5px] focus:outline-none sm:text-sm">
                    {items.map((item: Item) => (
                        <div
                            key={item.value}
                            className={`
              ...
              `}
                        ...
                        >
                            ...
                            {item.value === value && <CheckIcon className="shrink-0 h-4 w-4" />}
                        </div>
                    ))}
                </div>
            </PortalToFollowElemContent>
